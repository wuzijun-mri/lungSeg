...
import numpy as np
import torch
from models.trainer import mae_trainer
from datasets.datasets import mae_dataset
import argparse
import random
import logging
...
parser.add_argument('--annealing_epoch', type=int, default=25)
parser.add_argument('--batch_size', type=int, default=4)
parser.add_argument('--lr', type=float, default=2e-4)
parser.add_argument('--save_per_epoch', type=int, default=25)
parser.add_argument('--print_per_iter', type=int, default=50)
...
parser.add_argument('--mask_patch', type=int, default=12)
...
if __name__ == '__main__':
    ...
    # load trainer
    trainer = mae_trainer(opts)

    # load data
    dataset = mae_dataset(opts)
    train_loader = torch.utils.data.DataLoader(dataset, batch_size=opts.batch_size, shuffle=True, num_workers=opts.n_workers)


    for epoch in range(opts.epoch + opts.annealing_epoch + 1):
        ...
        if epoch > opts.annealing_epoch: # annealing epoch: additional scheduler
